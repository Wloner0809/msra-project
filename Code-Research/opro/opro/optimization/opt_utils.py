...
def _bucketize_float(num, n_buckets=20):
    assert 0 <= num <= 1, "The given number must be between 0 and 1."
        ...
        if few_shot_qa_pairs:
            ...
            for idx in few_shot_index_list:
                ...
                if include_qa:  # when "Q:" and "A:" are present in the prompt
                    # TODO: 在这里加了Q:和A:的格式, load_bbh_task_data中没有加
...
def run_evolution(**kwargs):
    ...
    if dataset_name == "mmlu":
        ...
        for task_name in tasks_all:
            ...
            # 相当于提取出train_index数组中的元素, 使得元素大于等于start_index, 小于end_index
    ...
    for i_step in range(num_search_steps):
        ...
        if few_shot_qa_pairs:
            # TODO: 可以优化的地方, 就是利用ICL选取demonstrations
            # 这里用的方法有: 选出错次数最多的QA pairs、当前instruction出错最频繁的QA pairs、
            # 每步固定QA pairs、每步都随机选取QA pairs
            if few_shot_selection_criteria == "accumulative_most_frequent":
                ...
                # 这里体现出是single step, 也就是"current most frequent"
        ...
        for instruction in generated_instructions:
            # TODO: 这里选择了跳过太长的指令、包含数字的指令、包含"INS"的指令
