...
# __file__就是当前文件的路径
# os.path.realpath(__file__)就是当前文件的绝对路径
# os.path.dirname(os.path.realpath(__file__))就是当前文件的目录
...
def main(_):
    ...
    if scorer_llm_name == "text-bison":
        ...
        # functools.partial()函数：把一个函数的某些参数给固定住，返回一个新的函数
        ...
        # update()函数：把字典dict2的键/值对更新到dict里
    ...
    if dataset_name == "mmlu":
        ...
        raw_data = []
        # TODO: for now only check the first task
    ...
    np.random.seed(0)
    # TODO: np.random.choice()第一个参数是数组, 但是num_examples是一个整数
    # train_index = np.sort(
    #     np.array(
    #         np.random.choice(
    ...
    train_index = np.sort(
        np.array(
            np.random.choice(
                np.arange(num_examples), size=int(train_ratio * num_examples), replace=False
            )
        )
    )
    ...
    # 'constant'}
    # TODO: 这里就是ICL选用demonstrations的策略, 可以改进的地方
