...
# 解决rate limit问题, 其实后面time.sleep()也可以解决
@retry(wait=wait_random_exponential(multiplier=1, min=60, max=100), stop=stop_after_attempt(5))
...
def call_openai_server_func(
    ...
    # 正常情况下, inputs是一个字符串列表, 但是也可能是一个字符串
