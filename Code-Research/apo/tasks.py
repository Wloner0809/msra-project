...
class DataProcessor(ABC):
    ...
    # TODO: 每个task类的get_train_examples和get_test_examples都是要修改的地方
    @abstractmethod
...
class EthosBinaryTask(BinaryClassificationTask):
    ...
    def get_train_examples(self):
        ...
        df = df[(df[1] <= 0) | (df[1] >= 0.7)]  # 筛选出第二列小于等于0或者大于等于0.7的行
        exs = df.reset_index().to_dict('records')  # 转换为由字典组成的列表, 每个字典对应一行, key为列名, value为对应的值
